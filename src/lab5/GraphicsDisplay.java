...
public class GraphicsDisplay extends JPanel {
    ...
    public GraphicsDisplay() {
        ...
        float[] dashArray=new float[]{20,5,20,5,20,5,
                5,5,5,5,5,5};
        this.graphicsStroke = new BasicStroke(2.0f, BasicStroke.CAP_BUTT,
    ...
    private void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point : this.graphicsData) {
            ...
            canvas.draw(marker);
            canvas.draw(new Line2D.Double(new Point2D.Double(center.getX()-radius, center.getY()-radius),
                    new Point2D.Double(center.getX()+radius, center.getY()+radius)));
            canvas.draw(new Line2D.Double(new Point2D.Double(center.getX()+radius, center.getY()-radius),
                    new Point2D.Double(center.getX()-radius, center.getY()+radius)));
        }
        if (lastMarker != null) {
